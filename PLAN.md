...
- [antfu/qrcode-toolkit](https://github.com/antfu/qrcode-toolkit) — DM Sans/Mono + `*-base` shortcuts; source of the collapsible **min↔random-range** behavior folded into `FormSlider` (`SettingsRandomRange` + `OptionSlider`/`OptionItem` settings rows)
