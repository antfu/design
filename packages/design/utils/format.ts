...
/**
 * Format a number with US-English grouping via `Intl.NumberFormat`.
 *
...
/**
 * Format a 0–1 fraction as a US-English percentage.
 *
...
const KB = 1024
const MB = 1024 * KB
const BYTES_SCALE: SeverityScale = [
 ...
 *
 * Thresholds: ≤80 KB neutral, ≤500 KB low, ≤1 MB medium, ≤10 MB high, else critical.
 *
 ...
 */
export function getBytesColor(bytes: number): ColorScaleClass {
  return mapSeverity(bytes, BYTES_SCALE)
}

// ── Durations ───────────────────────────────────────────────────────────────

/**
 * Humanize a millisecond duration → `[value, unit]`.
 *
 * Scales through `ms` → `s` → `min` → `h` → `d` and returns the value and unit
 * separately. Sub-millisecond durations render as `['<1', 'ms']`, and a
 * `null`/`undefined` input renders as `['', '-']`.
 *
 * @param ms - The duration in milliseconds, or `null`/`undefined` for no value.
 ...
 * formatDuration(90_000) // → ['1.5', 'min']
 */
export function formatDuration(ms: number | null | undefined): [string, string] {
  if (ms == null)
    return ['', '-']
  if (ms < 1)
    return ['<1', 'ms']
...
const DURATION_SCALE: SeverityScale = [
 ...
 *
 * Thresholds: ≤50 ms neutral, ≤200 ms low, ≤1000 ms medium, ≤5000 ms high, else critical.
 *
 ...
 */
export function getDurationColor(ms: number): ColorScaleClass {
  return mapSeverity(ms, DURATION_SCALE)
}
 ...
 *
 * Thresholds: ≤180 d neutral, ≤365 d low, ≤3 y medium, ≤5 y high, else critical.
 *
 ...
 */
export function getAgeColor(ageMs: number): ColorScaleClass {
  return mapSeverity(ageMs, AGE_SCALE)
}

/**
 * Format a date/time with US-English locale via `Intl.DateTimeFormat`.
 *
 ...
 */
export function formatDateTime(input: Date | number, options?: Intl.DateTimeFormatOptions): string {
